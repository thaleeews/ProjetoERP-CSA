...
{
    public partial class FrmPesquisaForn : Form
    {
        public FrmPesquisaForn()
        {
            InitializeComponent();
        }
        ...
        private void FrmPesquisaForn_Load(object sender, EventArgs e)
        {
            ...
            cmdQry.CommandText = "SELECT * FROM tblFornecedores";
            ...
            try
            {
                ...
                dgvForn.DataSource = bindingSource;
            }
            catch (Exception ex)
            {
        ...
        private void txtRazao_KeyUp(object sender, KeyEventArgs e)
        {
            string strConnection = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + Application.StartupPath + @"\dbProjectA.mdb";
            OleDbConnection dbConnection = new OleDbConnection(strConnection);
            OleDbCommand cmdQry = dbConnection.CreateCommand();
            ...
            // Definir a instrução SQL com Parâmetros
            cmdQry.CommandText = "SELECT * FROM [tblFornecedores] WHERE [nomeForn] LIKE @nome;";
            ...
            try
            {
                ...
                adapt = new OleDbDataAdapter(cmdQry.CommandText, dbConnection);
                construtor = new OleDbCommandBuilder(adapt);
                string param = txtRazao.Text.ToString();
        ...
        private void dgvForn_DoubleClick(object sender, EventArgs e)
        {
            if (dgvForn.SelectedRows.Count == 1 || dgvForn.SelectedCells.Count == 1)
            {
                var message = MessageBox.Show("Deseja visualizar o fornecedor " + dgvForn.CurrentRow.Cells[1].Value.ToString() + "?", "Confirme a ação", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (message == DialogResult.Yes)
                {
                    FrmFornecedor.pesquisa = dgvForn.CurrentRow.Cells[1].Value.ToString();
                    FrmFornecedor.idpesquisa = Convert.ToInt32(dgvForn.CurrentRow.Cells[0].Value);
                    Close();
                }
            }
        }
